...
class KeypointSelection:
    ...
    def traverse_density(self):
        pcd = o3d.io.read_point_cloud(path)  # 读取点云数据
        print(pcd)
        ...
        for i in range(pointNum):
            if i % 5000 == 0:
                print("候选点选择，进度" + str(round(i/1208.83,3)) + "%")
    ...
    def structure_tensor(self):
        ...
        for i in range(len(selected_index)):
            [k, idx, _] = pcd_tree.search_knn_vector_3d(pcd.points[selected_index[i]], self.k_neighbor)  # k近邻
    ...
    def combinatorially_geometric_characteristic(self):
        scatter_w = 1.5  # scatter相较于linear的权重
        ...
        geometrical_feature = np.zeros((number_of_point, 4))                       # 第i行是候选点中第i个点的linear, planar, scatter
